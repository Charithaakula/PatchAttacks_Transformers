...
def MultiPatchGDAttack(model, img, label, loss=nn.CrossEntropyLoss(), iterations=40, device=torch.device('cuda:0'), max_num_patches=100, clip_flag=False, bounds=[-1, 1], patch_size=16, lr=0.033, epsilon=1.0, *args, **kwargs):
   ...
   # patch_size = 16
    ...
    print(grad_val_patches.shape)
    xx = torch.norm(grad_val_patches,dim=(3,4),p="fro")
    grad_val_patches_norm = torch.norm(xx,dim=(2),p="fro")
    check = True
    print(grad_val_patches_norm[0][0])
    print(grad_mags[(0,0)])
    for i in range(0, int(sx/patch_size)):
        for j in range(0, int(sy/patch_size)):
            #print('entered for loop')
            x1 = grad_mags[(i*patch_size,j*patch_size)]
            x2 = grad_val_patches_norm[i][j]
            if not (math.isclose(x1,x2,rel_tol=1e-6)):
            #if (abs(x1-x2) < 1e-5):   
                check = False
                print(f'theese are unequal here at:{i} {j} {x1} {x2}')
                
    print('value of check is')
    print(check)
    ...
    for k in range(1, max_num_patches+1):
        ...
        img2 = copy.deepcopy(base_img).requires_grad_(
            True)  # Resetting image after each failure            
        for i in range(iterations):
            ...
            get_indices_repeat = get_indices_sorted.unsqueeze(2).unsqueeze(3).unsqueeze(4).repeat(1,1,3,16,16)  
            print(grad_val_patches.shape)              
            new_grad_val_patches = rearrange(grad_val[0], '1 c (l ps1) (w ps2) -> l w c ps1 ps2', ps1=patch_size, ps2 = patch_size)            
            #yy = torch.norm(new_grad_val_patches,dim=(3,4),p="fro")
            #new_grad_val_patches_norm = torch.norm(yy,dim=(2),p="fro")                        
            fil_grad_val_patches = new_grad_val_patches*get_indices_repeat
            print(fil_grad_val_patches.shape)
            fil_grad_val = rearrange(fil_grad_val_patches, 'x1 x2 c ps1 ps2 -> 1 c (x1 ps1) (x2 ps2)')                 
            print(fil_grad_val.shape)
            img2.data += lr*(fil_grad_val.sign()) 
            print('final image checks')           
            #print(img[0][0][176][176]==img2[0][0][176][176])                        
            #print(img[0][0][176]==img2[0][0][176])
            print(torch.equal(img, img2))
            print(torch.allclose(img, img2))
            print('ended comparision')
            #print()
            if clip_flag:
...
if __name__ == '__main__':
    ...
    print(args.outdir)
    outdir = args.outdir + "/" + str(args.mtype) +"/mt_" + str(args.mtype) + "_it_" + str(args.iteration) + "_mp_" + str(args.max_patches)  + "_ni_" + str(args.num_images) +  "_lr_" + str(args.lr) + "_ps_" + str(args.patch_size) + "_" + str(args.start_idx) + "_eps_" + str(args.epsilon)
    print(outdir)
